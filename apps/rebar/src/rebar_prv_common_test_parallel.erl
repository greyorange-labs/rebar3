...
symlink_to_last_ct_logs(State, Opts) ->
    LogDir = case Opts of
        {spec, _} ->
            %% Called with spec tuple instead of options list, use default logdir
            filename:join([rebar_dir:base_dir(State), "logs"]);
        _ when is_list(Opts) ->
            case proplists:get_value(logdir, Opts) of
